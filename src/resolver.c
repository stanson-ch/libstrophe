...
#define MESSAGE_TYPE_SRV 33
#define MESSAGE_CLASS_IN 1

/* returns 16 bit starting from the ptr */
#define PU16(ptr) (*(uint16_t *)(ptr))
...
static uint16_t xmpp_ntohs(uint16_t netshort)
{
    /* TODO implement independent solution */
                            ...
                            unsigned short *port)
{
    ...
    header.id = xmpp_ntohs(PU16(&buf[0]));
    header.octet2 = buf[2];
    header.octet3 = buf[3];
    header.qdcount = xmpp_ntohs(PU16(&buf[4]));
    header.ancount = xmpp_ntohs(PU16(&buf[6]));
    header.nscount = xmpp_ntohs(PU16(&buf[8]));
    header.arcount = xmpp_ntohs(PU16(&buf[10]));
    ...
    for (i = 0; i < header.ancount; ++i) {
        name_len = message_name_len(buf, len, j);
        j += name_len;
        type = xmpp_ntohs(PU16(&buf[j]));
        class = xmpp_ntohs(PU16(&buf[j + 2]));
        rdlength = xmpp_ntohs(PU16(&buf[j + 8]));
        j += 10;
        if (type == MESSAGE_TYPE_SRV && class == MESSAGE_CLASS_IN) {
            priority = xmpp_ntohs(PU16(&buf[j]));
            if (!set || priority < priority_min) {
                *port = xmpp_ntohs(PU16(&buf[j + 4]));
                        ...
                        size_t target_len, unsigned short *port)
{
    ...
    return resolver_srv_lookup_buf(buf, (size_t)len, target, target_len, port);
}
